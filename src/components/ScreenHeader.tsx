...
export function ScreenHeader({ onBack, rightElement, subtitle, title }: ScreenHeaderProps) {
  return (
    <View className="border-b border-[#EEF1FF] bg-[#F9F8FF]/95 px-4 pb-4 pt-6">
      <View className="flex-row items-center gap-3">
        ...
        {rightElement ? (
          <View className="h-10 w-10 items-center justify-center rounded-full bg-surface shadow-sm">{rightElement}</View>
        ) : null}
      </View>
    </View>
  );
